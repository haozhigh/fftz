...
int main(int argc, char** argv){
	...
	for(n = 4, i = 2; n <= N; n *= 4, i+=2){
		printf("gpuFFTFour %5d %10d start...\n", i, n);
		least_time = gpuFFTFour(&cl, n, data0);
		for(j = 0; j < repeat - 1; j++){
			current_time = gpuFFTFour(&cl, n, data0);
			if(current_time < least_time)	least_time = current_time;
		}
		time[i] = least_time;
	}
	printf("Writing results to file...\n");
	writeResult(time, "./output/lengthFlopsFour");
